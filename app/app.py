...
chlorella_detection_model = model_builder.build(model_config=chlorella_configs['model'], is_training=False)

# Restore checkpoint
chlorella_ckpt = tf.compat.v2.train.Checkpoint(model=chlorella_detection_model)
...
def chlorella_fn(image):
    image, shapes = chlorella_detection_model.preprocess(image)
    prediction_dict = chlorella_detection_model.predict(image, shapes)
    detections = chlorella_detection_model.postprocess(prediction_dict, shapes)
    return detections
...
category_index = label_map_util.create_category_index_from_labelmap(os.path.join("app", "static", "diatom", "label_map.pbtxt"))
...
def detect_cells_opencv(image_rgb, min_area=80, max_area=10000, split_aspect_ratio=1.6):
    """Detect colored algae cells via HSV saturation segmentation.

    Elongated contours (aspect ratio >= split_aspect_ratio) are treated as two
    dividing cells and split into two boxes.
    """
    h, w = image_rgb.shape[:2]
    hsv = cv2.cvtColor(image_rgb, cv2.COLOR_RGB2HSV)

    # Blue-green algae only: hue 70-135 excludes brown/yellow debris and grid lines
    lower = np.array([70, 30, 30])
    upper = np.array([135, 255, 230])
    mask = cv2.inRange(hsv, lower, upper)

    # OPEN only — removes noise without merging adjacent cells
    ...
    for c in contours:
        ...
        if aspect >= split_aspect_ratio:
            # Dividing cell clearly split into two spheres → count as two
            if cw >= ch:
                mid_x = x + cw / 2
                boxes.append([y / h, x / w, (y + ch) / h, mid_x / w])
                boxes.append([y / h, mid_x / w, (y + ch) / h, (x + cw) / w])
            else:
                mid_y = y + ch / 2
                boxes.append([y / h, x / w, mid_y / h, (x + cw) / w])
                boxes.append([mid_y / h, x / w, (y + ch) / h, (x + cw) / w])
        else:
            boxes.append([y / h, x / w, (y + ch) / h, (x + cw) / w])
...
def suppress_boxes(detections, confidence_threshold=0.1, iou_threshold=0.5):
    ...
    all_boxes = list(enumerate(detections["detection_boxes"]))

    final_boxes = sorted(
        [bbox for bbox in all_boxes
         if detections["detection_scores"][bbox[0]] > confidence_threshold
         and float(calc_box_area(bbox[1])) < 0.01],
        key=lambda bbox: detections["detection_scores"][bbox[0]],
        reverse=True
    )
    remaining_bboxes = []
    while final_boxes:
...
def count_concentration_detections(image, cell_type, image_name, threshold=0.1, cell_volume=271.8, cell_length=16, depth=0.1):
    if cell_type == 'chlorella':
        ...
        all_boxes = detect_cells_opencv(img_patch)
        scaled_boxes = list(map(lambda x: [x[0] * img_height, x[1] * img_width, x[2] * img_height, x[3] * img_width], all_boxes))
        patch_results = {
            "patch": img_patch,
            "detections": np.array(all_boxes) if all_boxes else np.zeros((0, 4)),
        }
        estimated_cell_count = len(scaled_boxes)
        concentration = chlamy_concentration(scaled_boxes, total_area, cell_volume / 2, depth)
...
@app.route("/synopsis")
def return_synopsis():
    return send_file(os.path.join("static", "files", 'synopsis.pdf'), attachment_filename='Synopsis.pdf')
